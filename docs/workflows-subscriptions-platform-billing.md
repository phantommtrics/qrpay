...
- Checkout flows use **platform-configured** gateways and webhooks (`subscription-invoice-checkout.service.ts`, `wave-subscription-webhook.service.ts`, `yonna-subscription-webhook.service.ts`). Wave uses one portal key (`WAVE_CHECKOUT_BEARER`). Wave aggregator keys require `aggregated_merchant_id` on every checkout: subscription invoices use the **main merchant** id (`WAVE_PLATFORM_AGGREGATED_MERCHANT_ID` or auto `"<PLATFORM_NAME> Platform"`); merchant sales use the tenant aggregated merchant id.
