...
When creating an order at checkout time, you may include an optional **`category`** string in the JSON body of `POST …/businesses/{businessId}/orders`. Use it to tag what the payment is for in your own taxonomy (booking type, product line, etc.). Catalogue SKUs stay in the partner app — Easypay does not require you to create products. Easypay uppercases the text, finds or creates a matching **sales category**, binds it to the order (and therefore the payment), returns it as `category` on the order object, and includes `category` on outbound webhooks (`payment.completed`, `payment.cancelled`, `payment.failed`) when set. No change is required if you do not need segmentation — existing integrations without `category` continue to work.
