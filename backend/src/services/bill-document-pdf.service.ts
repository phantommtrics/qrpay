...
export type BillPdfRow = Prisma.BillGetPayload<{ include: typeof billForPdfInclude }>;
...
export function buildBillPdfBuffer(row: BillPdfRow): Promise<Buffer> {
  return pdfBufferFromDoc((doc, margin, contentRight) => {
    ...
    doc.text(
      "This purchase bill records amounts owed to your supplier. Please retain a copy for your files.",
      margin,
      y,
...
export async function loadBillForPdf(businessId: string, billId: string): Promise<BillPdfRow> {
  const row = await prisma.bill.findFirst({
    where: { id: billId, businessId },
    include: billForPdfInclude,
  });
  if (!row) {
...
export async function loadBillForPdfById(billId: string): Promise<BillPdfRow | null> {
...
export async function renderBillPdfDownload(
  businessId: string,
  billId: string,
): Promise<{ buffer: Buffer; filename: string }> {
  const row = await loadBillForPdf(businessId, billId);
  const buffer = await buildBillPdfBuffer(row);
