...
import {
  ...
  type PlatformBillBulkPostPreviewItem,
  type PlatformBillBulkPostResult,
...
type Props = {
  ...
  onClose: () => void
  onComplete?: (summary: PlatformBillBulkPostSummary) => void
}
...
export function PlatformBillBulkPostModal({
  open,
  billIds,
  gateways,
  onClose,
  ...
  const [previewItems, setPreviewItems] = useState<PlatformBillBulkPostPreviewItem[]>([])
  const [results, setResults] = useState<PlatformBillBulkPostResult[]>([])
  const [resultSummary, setResultSummary] = useState<PlatformBillBulkPostSummary | null>(null)
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const chart = await fetchPlatformAccountingChart()
        const assets = chart.filter((a) => a.category === 'ASSET')
        setAccounts(assets)
  ...
  const handleExecute = async () => {
    ...
    setError(null)
    try {
      const summary = await executePlatformBillBulkPost({
        billIds,
        gatewayCode,
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" role="presentation">
      ...
      <CenteredModal className="relative z-10 flex max-h-[90vh] w-full max-w-3xl flex-col overflow-hidden rounded-md border border-qb-border bg-white shadow-2xl">
        <div className="border-b border-qb-border px-5 py-4">
          <h2 className="text-lg font-semibold text-qb-heading">Bulk post supplier bills</h2>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4">
          ...
          {step === 'gateway' ? (
            <div className="space-y-3">
              <p className="text-sm text-qb-muted">Choose a payment gateway (subscription checkout).</p>
              {gateways.length === 0 ? (
                <p className="text-sm text-qb-muted">No payment gateways are configured.</p>
              ) : null}
          ...
          {step === 'settlement' ? (
            <div className="space-y-4">
              <p className="text-sm text-qb-muted">
                Settlement account for the platform ledger entry when bills are marked paid.
              </p>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                {results.map((r) => {
                  ...
                  const supplier = r.supplierName ?? item?.supplierName
                  const phone = r.supplierPhone ?? item?.supplierPhone
                    ...
                    >
                      <div className="flex flex-wrap items-start justify-between gap-2">
                        <div>
                          ...
                          {supplier ? (
                            <span className="ml-2 text-qb-muted">{supplier}</span>
                          ) : null}
                        </div>
                        {r.success ? (
