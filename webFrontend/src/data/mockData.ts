import type {
  AccountingTransaction,
  CashAccountBalance,
  ChartOfAccountEntry,
  DashboardStats,
  Order,
  Organization,
  Payment,
  PermissionDefinition,
  PlanPermissions,
  ProfitLossSnapshot,
  ProfitLossTrendPoint,
  SubscriptionPlan,
} from '../types'

export const SUBSCRIPTION_PLANS: SubscriptionPlan[] = [
  {
    id: 'basic',
    name: 'Basic',
    priceLabel: 'D1,500 / month',
    yearlyPriceLabel: 'D15,000 / year',
    staffLabel: '3-5 staff',
    minStaff: 3,
    maxStaff: 5,
    description: 'Best for small shops and cafes launching QR-based checkout.',
  },
  {
    id: 'pro',
    name: 'Pro',
    priceLabel: 'D3,500 / month',
    yearlyPriceLabel: 'D35,000 / year',
    staffLabel: '6-10 staff',
    minStaff: 6,
    maxStaff: 10,
    description: 'Built for growing teams that need more operations and reporting access.',
    highlighted: true,
  },
  {
    id: 'business_pro',
    name: 'Business Pro',
    priceLabel: 'Custom pricing',
    yearlyPriceLabel: 'Custom pricing',
    staffLabel: 'Unlimited staff',
    minStaff: 11,
    maxStaff: null,
    description: 'For larger businesses and enterprises that need full platform coverage.',
  },
  {
    id: 'corporate',
    name: 'Corporate',
    priceLabel: 'Custom pricing',
    yearlyPriceLabel: 'Custom pricing',
    staffLabel: 'Unlimited staff',
    minStaff: 1,
    maxStaff: null,
    description:
      'Corporate industry default: full operations except POS, products, and orders (categories stay available); pricing is set by platform staff.',
  },
]

export const PERMISSION_DEFINITIONS: PermissionDefinition[] = [
  // Platform permissions
  {
    key: 'platform.manage',
    label: 'Platform Management',
    description: 'Manage platform settings and configurations',
    category: 'Administration',
  },
  {
    key: 'platform.users.manage',
    label: 'Platform Users',
    description: 'Manage platform-level users',
    category: 'Administration',
  },
  {
    key: 'platform.businesses.manage',
    label: 'Platform Businesses',
    description: 'Manage all businesses on platform',
    category: 'Administration',
  },
  {
    key: 'platform.wave_operations.view',
    label: 'Wave operations (view)',
    description: 'View Wave wallet balance, transactions, and payout history',
    category: 'Administration',
  },
  {
    key: 'platform.wave_operations.manage',
    label: 'Wave operations (manage)',
    description: 'Send Wave payouts, reverse payouts, and refund transactions',
    category: 'Administration',
  },
  {
    key: 'platform.business_merchants.view',
    label: 'Business merchants (view)',
    description: 'View cross-merchant journal, 360 journal, fee, and self settlement reports',
    category: 'Administration',
  },
  {
    key: 'platform.business_merchants.export',
    label: 'Business merchants (export)',
    description: 'Export business merchants reports as CSV or PDF',
    category: 'Administration',
  },
  {
    key: 'platform.digitalocean_billing.view',
    label: 'DigitalOcean billing (view)',
    description: 'View DigitalOcean balance, invoices, and billing history',
    category: 'Administration',
  },
  {
    key: 'platform.digitalocean_billing.manage',
    label: 'DigitalOcean billing (post journal)',
    description: 'Post DigitalOcean invoices to the platform journal as hosting expenses',
    category: 'Administration',
  },
  {
    key: 'platform.businesses.merchant_api.view',
    label: 'Businesses — Merchant API (view)',
    description: 'View wallet integration status and webhooks for any business (platform business detail)',
    category: 'Administration',
  },
  {
    key: 'platform.businesses.merchant_api.edit',
    label: 'Businesses — Merchant API (edit)',
    description: 'Add or change merchant wallet credentials and clear APS customer auths on behalf of a business',
    category: 'Administration',
  },
  {
    key: 'platform.subscriptions.view',
    label: 'Platform subscriptions list',
    description: 'View subscription rows across businesses',
    category: 'Administration',
  },
  {
    key: 'platform.invoices.view',
    label: 'Platform invoices',
    description: 'View platform billing invoices',
    category: 'Administration',
  },
  {
    key: 'platform.invoices.export',
    label: 'Platform invoices export',
    description: 'Invoices module export flag (legacy; billing ledger CSV also honors Billing transactions → Export)',
    category: 'Administration',
  },
  {
    key: 'platform.billing_transactions.view',
    label: 'Billing transactions',
    description: 'Cross-business subscription billing ledger report',
    category: 'Administration',
  },
  {
    key: 'platform.billing_transactions.export',
    label: 'Billing transactions export',
    description: 'Export billing ledger as CSV (all businesses)',
    category: 'Administration',
  },
  {
    key: 'platform.billing_review.view',
    label: 'Billing review (view)',
    description: 'List subscription invoices and payment ledger rows for refund decisions',
    category: 'Administration',
  },
  {
    key: 'platform.billing_review.edit',
    label: 'Billing review (edit)',
    description: 'Set manual refund review flags (no money movement in-app)',
    category: 'Administration',
  },
  {
    key: 'platform.billing.manage',
    label: 'Platform plan billing',
    description: 'Set monthly and yearly subscription prices (platform billing module in role templates)',
    category: 'Administration',
  },
  {
    key: 'platform.payment_gateways.manage',
    label: 'Payment gateways',
    description: 'Enable Wave and other providers for business subscription checkout',
    category: 'Administration',
  },
  {
    key: 'platform.system.view',
    label: 'System configuration',
    description: 'View platform catalog, services, products, and plan entitlements',
    category: 'Administration',
  },
  {
    key: 'platform.security.roles.view',
    label: 'Security — Role templates',
    description: 'Define access templates per platform module',
    category: 'Administration',
  },
  {
    key: 'platform.security.function_groups.view',
    label: 'Security — Function groups',
    description: 'Bundle role templates into assignable permission sets',
    category: 'Administration',
  },
  {
    key: 'platform.security.users.view',
    label: 'Security — System users',
    description: 'Create and assign platform staff accounts',
    category: 'Administration',
  },
  {
    key: 'platform.security.move_users.view',
    label: 'Security — Move users',
    description: 'Open Move users; grant Edit on this module to run bulk moves between groups',
    category: 'Administration',
  },
  {
    key: 'platform.security.partnership_config.view',
    label: 'Security — Partnership config',
    description: 'Manage internal partner outbound webhook URLs and signing secrets',
    category: 'Administration',
  },

  // Business permissions
  {
    key: 'business.manage',
    label: 'Business Management',
    description: 'Manage business settings and configurations',
    category: 'Administration',
  },
  {
    key: 'staff.manage',
    label: 'Staff Management',
    description: 'Manage business staff and roles',
    category: 'Administration',
  },
  {
    key: 'status.change.view',
    label: 'Staff access status',
    description: 'View and change member access (active, blocked, suspended, terminated)',
    category: 'Administration',
  },
  {
    key: 'products.manage',
    label: 'Products Management',
    description: 'Full CRUD operations on products',
    category: 'Products',
  },
  {
    key: 'products.view',
    label: 'Products View',
    description: 'View products catalog',
    category: 'Views',
  },
  {
    key: 'products.barcode',
    label: 'Product Barcode',
    description: 'Restaurant manual menu grid with product barcodes and PDF export',
    category: 'Products',
  },
  {
    key: 'products.create',
    label: 'Products Create',
    description: 'Add new products into the catalogue',
    category: 'Actions',
  },
  {
    key: 'products.edit',
    label: 'Products Edit',
    description: 'Update existing products',
    category: 'Actions',
  },
  {
    key: 'products.delete',
    label: 'Products Delete',
    description: 'Delete products from the business catalogue',
    category: 'Actions',
  },
  {
    key: 'orders.manage',
    label: 'Orders Management',
    description: 'Manage orders and transactions',
    category: 'Actions',
  },
  {
    key: 'orders.view',
    label: 'Orders View',
    description: 'View orders and transactions',
    category: 'Views',
  },
  {
    key: 'payments.manage',
    label: 'Payments Management',
    description: 'Manage payment records',
    category: 'Actions',
  },
  {
    key: 'payments.view',
    label: 'Payments View',
    description: 'View payment records',
    category: 'Views',
  },
  {
    key: 'payments.export',
    label: 'Payments Export',
    description: 'Export payment data',
    category: 'Reports',
  },
  {
    key: 'reports.view',
    label: 'Reports View',
    description: 'Access business reports',
    category: 'Reports',
  },
  {
    key: 'reports.export',
    label: 'Reports Export',
    description: 'Export report data',
    category: 'Reports',
  },
  {
    key: 'accounting.view',
    label: 'Accounting View',
    description: 'Access accounting features',
    category: 'Reports',
  },
  {
    key: 'accounting.chart.view',
    label: 'Chart of Accounts',
    description: 'View chart of accounts',
    category: 'Reports',
  },
  {
    key: 'accounting.reports.gl',
    label: 'GL balance report',
    description: 'Trial balance / GL balances as at a date',
    category: 'Reports',
  },
  {
    key: 'accounting.reports.pnl',
    label: 'Profit & loss report',
    description: 'Income and expenses for a date range',
    category: 'Reports',
  },
  {
    key: 'accounting.reports.balance_sheet',
    label: 'Balance sheet',
    description: 'Assets, liabilities, and equity as at a date',
    category: 'Reports',
  },
  {
    key: 'accounting.reports.statement',
    label: 'Account statement',
    description: 'Activity for a single ledger account',
    category: 'Reports',
  },
  {
    key: 'accounting.reports.export',
    label: 'Export finance reports',
    description: 'PDF and CSV export for finance reports',
    category: 'Reports',
  },
  {
    key: 'accounting.journals.general',
    label: 'General journal',
    description: 'Post balanced debit/credit entries without cash or bank settlement',
    category: 'Reports',
  },
  {
    key: 'accounting.journals.reversal',
    label: 'Reversed journal',
    description: 'Post an opposite journal to undo an incorrect ledger posting',
    category: 'Reports',
  },
  {
    key: 'accounting.transaction_journal',
    label: 'Transaction journal',
    description: 'Review and approve postings before they appear on GL, P&L, and statements',
    category: 'Reports',
  },
  {
    key: 'sales.quotation',
    label: 'Sales quotations',
    description: 'Create and manage customer quotations',
    category: 'Sales',
  },
  {
    key: 'sales.invoice',
    label: 'Sales invoices',
    description: 'Create and manage sales invoices and payment posting',
    category: 'Sales',
  },
  {
    key: 'sales.bill',
    label: 'Bills',
    description: 'Supplier purchase bills and payment posting (money out)',
    category: 'Sales',
  },
  {
    key: 'sales.settlement',
    label: 'DirectPay settlement',
    description: 'Request payout of digital collections',
    category: 'Sales',
  },
  {
    key: 'pos.access',
    label: 'POS Access',
    description: 'Access point of sale system',
    category: 'Operations',
  },
  {
    key: 'dashboard.view',
    label: 'Dashboard View',
    description: 'Access business dashboard',
    category: 'Views',
  },
  {
    key: 'organization.manage',
    label: 'Organization Settings',
    description: 'Update organization profile and subscription settings.',
    category: 'Administration',
  },
  {
    key: 'contacts.manage',
    label: 'Contacts',
    description: 'View and create business contacts for journals and sales documents',
    category: 'Administration',
  },
  {
    key: 'business.configuration',
    label: 'Access configuration',
    description: 'Assign subscription features to team members.',
    category: 'Administration',
  },
  {
    key: 'subscriptions.billings',
    label: 'Billing',
    description: 'Pay subscription invoices and manage payment methods',
    category: 'Administration',
  },
  {
    key: 'subscriptions.invoices',
    label: 'Subscription invoices',
    description: 'View subscription billing history for this business',
    category: 'Administration',
  },
  {
    key: 'subscriptions.billing_activity',
    label: 'Subscription payments',
    description: 'Plan payment activity from the billing ledger',
    category: 'Administration',
  },
  {
    key: 'merchant.api',
    label: 'Merchant API',
    description: 'Configure encrypted Wave and Yonna credentials for customer payments',
    category: 'Integrations',
  },
  {
    key: 'merchant.profile',
    label: 'Profile',
    description: 'Business profile, document logo, and account password',
    category: 'Integrations',
  },
  {
    key: 'activity.log',
    label: 'Activity log',
    description: 'View business audit trail: payments, product changes, and staff actions',
    category: 'Administration',
  },
]

export const INITIAL_PLAN_PERMISSIONS: PlanPermissions = {
  basic: {
    'dashboard.view': true,
    'pos.access': true,
    'products.view': true,
    'products.barcode': true,
    'products.create': true,
    'products.edit': true,
    'products.delete': false,
    'products.manage': false,
    'orders.view': true,
    'orders.manage': true,
    'payments.view': true,
    'payments.manage': true,
    'payments.export': false,
    'reports.view': false,
    'reports.export': false,
    'accounting.view': false,
    'accounting.chart.view': false,
    'accounting.reports.gl': false,
    'accounting.reports.pnl': false,
    'accounting.reports.balance_sheet': false,
    'accounting.reports.statement': false,
    'accounting.reports.export': false,
    'accounting.journals.general': false,
    'accounting.journals.reversal': false,
    'accounting.transaction_journal': false,
    'sales.quotation': false,
    'sales.invoice': false,
    'sales.bill': false,
    'sales.settlement': false,
    'staff.manage': true,
    'status.change.view': true,
    'organization.manage': true,
    'business.manage': true,
    'business.configuration': true,
    'subscriptions.billings': true,
    'subscriptions.invoices': true,
    'subscriptions.billing_activity': true,
    'merchant.api': true,
    'merchant.profile': true,
    'contacts.manage': true,
    'activity.log': true,
  },
  pro: {
    'dashboard.view': true,
    'pos.access': true,
    'products.view': true,
    'products.barcode': true,
    'products.create': true,
    'products.edit': true,
    'products.delete': false,
    'products.manage': false,
    'orders.view': true,
    'orders.manage': true,
    'payments.view': true,
    'payments.manage': true,
    'payments.export': true,
    'reports.view': true,
    'reports.export': false,
    'accounting.view': true,
    'accounting.chart.view': false,
    'accounting.reports.gl': true,
    'accounting.reports.pnl': true,
    'accounting.reports.balance_sheet': true,
    'accounting.reports.statement': true,
    'accounting.reports.export': true,
    'accounting.journals.general': true,
    'accounting.journals.reversal': true,
    'accounting.transaction_journal': true,
    'sales.quotation': true,
    'sales.invoice': true,
    'sales.bill': true,
    'sales.settlement': true,
    'staff.manage': true,
    'status.change.view': true,
    'organization.manage': true,
    'business.manage': true,
    'business.configuration': true,
    'subscriptions.billings': true,
    'subscriptions.invoices': true,
    'subscriptions.billing_activity': true,
    'merchant.api': true,
    'merchant.profile': true,
    'contacts.manage': true,
    'activity.log': true,
  },
  business_pro: {
    'dashboard.view': true,
    'pos.access': true,
    'products.view': true,
    'products.barcode': true,
    'products.create': true,
    'products.edit': true,
    'products.delete': true,
    'products.manage': true,
    'orders.view': true,
    'orders.manage': true,
    'payments.view': true,
    'payments.manage': true,
    'payments.export': true,
    'reports.view': true,
    'reports.export': true,
    'accounting.view': true,
    'accounting.chart.view': true,
    'accounting.reports.gl': true,
    'accounting.reports.pnl': true,
    'accounting.reports.balance_sheet': true,
    'accounting.reports.statement': true,
    'accounting.reports.export': true,
    'accounting.journals.general': true,
    'accounting.journals.reversal': true,
    'accounting.transaction_journal': true,
    'sales.quotation': true,
    'sales.invoice': true,
    'sales.bill': true,
    'sales.settlement': true,
    'staff.manage': true,
    'status.change.view': true,
    'organization.manage': true,
    'business.manage': true,
    'business.configuration': true,
    'subscriptions.billings': true,
    'subscriptions.invoices': true,
    'subscriptions.billing_activity': true,
    'merchant.api': true,
    'merchant.profile': true,
    'contacts.manage': true,
    'activity.log': true,
  },
  corporate: {
    'dashboard.view': true,
    'pos.access': false,
    'products.view': false,
    'products.barcode': false,
    'products.create': false,
    'products.edit': false,
    'products.delete': false,
    'products.manage': false,
    'orders.view': false,
    'orders.manage': false,
    'payments.view': true,
    'payments.manage': true,
    'payments.export': true,
    'reports.view': true,
    'reports.export': true,
    'accounting.view': true,
    'accounting.chart.view': true,
    'accounting.reports.gl': true,
    'accounting.reports.pnl': true,
    'accounting.reports.balance_sheet': true,
    'accounting.reports.statement': true,
    'accounting.reports.export': true,
    'accounting.journals.general': true,
    'accounting.journals.reversal': true,
    'accounting.transaction_journal': true,
    'sales.quotation': true,
    'sales.invoice': true,
    'sales.bill': true,
    'sales.settlement': true,
    'staff.manage': true,
    'status.change.view': true,
    'organization.manage': true,
    'business.manage': true,
    'business.configuration': true,
    'subscriptions.billings': true,
    'subscriptions.invoices': true,
    'subscriptions.billing_activity': true,
    'merchant.api': true,
    'merchant.profile': true,
    'contacts.manage': true,
    'activity.log': true,
  },
}

export const MOCK_ORGANIZATIONS: Organization[] = [
  {
    id: 'b1',
    name: 'Sunrise Mart',
    slug: 'sunrise-mart',
    industry: 'Retail',
    planId: 'basic',
    staffCount: 5,
    ownerName: 'Fatou Jallow',
    subscriptionExpiresAt: '2026-04-22T00:00:00.000Z',
    createdAt: '2026-01-05T10:00:00.000Z',
  },
  {
    id: 'b2',
    name: 'Baobab Kitchen',
    slug: 'baobab-kitchen',
    industry: 'Restaurant',
    planId: 'pro',
    staffCount: 8,
    ownerName: 'Bakary Sanyang',
    subscriptionExpiresAt: '2026-03-29T00:00:00.000Z',
    createdAt: '2025-11-15T10:00:00.000Z',
  },
  {
    id: 'b3',
    name: 'Legacy Foods',
    slug: 'legacy-foods',
    industry: 'Wholesale',
    planId: 'business_pro',
    staffCount: 18,
    ownerName: 'Awa Camara',
    subscriptionExpiresAt: '2026-03-12T00:00:00.000Z',
    createdAt: '2025-09-10T10:00:00.000Z',
  },
]

export const MOCK_ORDERS: Order[] = [
  {
    id: 'ORD-8821',
    items: [
      {
        id: 'i1',
        productId: 'p1',
        productName: 'Coca-Cola 500ml',
        quantity: 2,
        price: 35,
      },
      {
        id: 'i2',
        productId: 'p3',
        productName: 'Plantain Chips',
        quantity: 1,
        price: 25,
      },
    ],
    status: 'completed',
    total: 95,
    businessId: 'b1',
    createdAt: new Date(Date.now() - 1000 * 60 * 5).toISOString(),
  },
  {
    id: 'ORD-8822',
    items: [
      {
        id: 'i3',
        productId: 'r1',
        productName: 'Grilled Chicken Yassa',
        quantity: 1,
        price: 350,
      },
      {
        id: 'i4',
        productId: 'r5',
        productName: 'Wonjo Juice',
        quantity: 2,
        price: 50,
      },
    ],
    status: 'preparing',
    total: 450,
    businessId: 'b2',
    tableId: 'T-04',
    createdAt: new Date(Date.now() - 1000 * 60 * 15).toISOString(),
  },
  {
    id: 'ORD-8823',
    items: [
      {
        id: 'i5',
        productId: 'p5',
        productName: 'Local Rice 5kg',
        quantity: 1,
        price: 450,
      },
    ],
    status: 'completed',
    total: 450,
    businessId: 'b1',
    createdAt: new Date(Date.now() - 1000 * 60 * 45).toISOString(),
  },
  {
    id: 'ORD-8824',
    items: [
      {
        id: 'i6',
        productId: 'r3',
        productName: 'Benachin (Jollof Rice)',
        quantity: 3,
        price: 250,
      },
    ],
    status: 'served',
    total: 750,
    businessId: 'b2',
    tableId: 'T-02',
    createdAt: new Date(Date.now() - 1000 * 60 * 60).toISOString(),
  },
  {
    id: 'ORD-8825',
    items: [
      {
        id: 'i7',
        productId: 'p2',
        productName: 'Bottled Water 1L',
        quantity: 5,
        price: 20,
      },
    ],
    status: 'pending',
    total: 100,
    businessId: 'b1',
    createdAt: new Date(Date.now() - 1000 * 60 * 2).toISOString(),
  },
]

export const MOCK_PAYMENTS: Payment[] = [
  {
    id: 'PAY-9910',
    orderId: 'ORD-8821',
    amount: 95,
    status: 'completed',
    reference: 'WLT-7738291',
    method: 'qr_wallet',
    businessId: 'b1',
    createdAt: new Date(Date.now() - 1000 * 60 * 5).toISOString(),
  },
  {
    id: 'PAY-9911',
    orderId: 'ORD-8823',
    amount: 450,
    status: 'completed',
    reference: 'CSH-0000001',
    method: 'cash',
    businessId: 'b1',
    createdAt: new Date(Date.now() - 1000 * 60 * 45).toISOString(),
  },
  {
    id: 'PAY-9912',
    orderId: 'ORD-8824',
    amount: 750,
    status: 'completed',
    reference: 'WLT-7738292',
    method: 'qr_wallet',
    businessId: 'b2',
    createdAt: new Date(Date.now() - 1000 * 60 * 60).toISOString(),
  },
  {
    id: 'PAY-9913',
    orderId: 'ORD-8825',
    amount: 100,
    status: 'pending',
    reference: 'WLT-7738293',
    method: 'qr_wallet',
    businessId: 'b1',
    createdAt: new Date(Date.now() - 1000 * 60 * 2).toISOString(),
  },
  {
    id: 'PAY-9914',
    orderId: 'ORD-8820',
    amount: 1200,
    status: 'failed',
    reference: 'WLT-7738290',
    method: 'qr_wallet',
    businessId: 'b2',
    createdAt: new Date(Date.now() - 1000 * 60 * 120).toISOString(),
  },
]

export const MOCK_STATS: DashboardStats = {
  totalSales: 12450,
  totalOrders: 48,
  totalProducts: 14,
  lowStockCount: 2,
}

export const REVENUE_DATA = [
  { name: 'Mon', revenue: 4000 },
  { name: 'Tue', revenue: 3000 },
  { name: 'Wed', revenue: 5500 },
  { name: 'Thu', revenue: 4500 },
  { name: 'Fri', revenue: 7000 },
  { name: 'Sat', revenue: 9500 },
  { name: 'Sun', revenue: 8200 },
]

export const CASH_ACCOUNT_BALANCES: CashAccountBalance[] = [
  {
    id: 'cash-b1-bank',
    businessId: 'b1',
    name: 'Trust Bank - Main Account',
    type: 'bank',
    balance: 186500,
    lastUpdatedAt: '2026-03-20T09:30:00.000Z',
  },
  {
    id: 'cash-b1-merchant',
    businessId: 'b1',
    name: 'DirectPay Merchant Wallet',
    type: 'merchant',
    balance: 28420,
    lastUpdatedAt: '2026-03-20T09:30:00.000Z',
  },
  {
    id: 'cash-b2-bank',
    businessId: 'b2',
    name: 'GTBank Operations Account',
    type: 'bank',
    balance: 264800,
    lastUpdatedAt: '2026-03-20T09:30:00.000Z',
  },
  {
    id: 'cash-b2-merchant',
    businessId: 'b2',
    name: 'DirectPay Merchant Wallet',
    type: 'merchant',
    balance: 51240,
    lastUpdatedAt: '2026-03-20T09:30:00.000Z',
  },
  {
    id: 'cash-b3-bank',
    businessId: 'b3',
    name: 'Enterprise Treasury Account',
    type: 'bank',
    balance: 824500,
    lastUpdatedAt: '2026-03-20T09:30:00.000Z',
  },
  {
    id: 'cash-b3-merchant',
    businessId: 'b3',
    name: 'DirectPay Merchant Wallet',
    type: 'merchant',
    balance: 132900,
    lastUpdatedAt: '2026-03-20T09:30:00.000Z',
  },
]

export const PROFIT_LOSS_SNAPSHOTS: ProfitLossSnapshot[] = [
  {
    businessId: 'b1',
    totalIncome: 382000,
    costOfSales: 214000,
    grossProfit: 168000,
    operatingExpenses: 73500,
    netProfit: 94500,
  },
  {
    businessId: 'b2',
    totalIncome: 548000,
    costOfSales: 287000,
    grossProfit: 261000,
    operatingExpenses: 112500,
    netProfit: 148500,
  },
  {
    businessId: 'b3',
    totalIncome: 1215000,
    costOfSales: 662000,
    grossProfit: 553000,
    operatingExpenses: 241300,
    netProfit: 311700,
  },
]

export const PROFIT_LOSS_TREND: ProfitLossTrendPoint[] = [
  { businessId: 'b1', period: 'Jan', income: 248000, expenses: 182000 },
  { businessId: 'b1', period: 'Feb', income: 291000, expenses: 206000 },
  { businessId: 'b1', period: 'Mar', income: 382000, expenses: 287500 },
  { businessId: 'b1', period: 'Apr', income: 401000, expenses: 295500 },
  { businessId: 'b2', period: 'Jan', income: 332000, expenses: 249000 },
  { businessId: 'b2', period: 'Feb', income: 396000, expenses: 286000 },
  { businessId: 'b2', period: 'Mar', income: 451000, expenses: 318000 },
  { businessId: 'b2', period: 'Apr', income: 548000, expenses: 399500 },
  { businessId: 'b3', period: 'Jan', income: 812000, expenses: 578000 },
  { businessId: 'b3', period: 'Feb', income: 934000, expenses: 651000 },
  { businessId: 'b3', period: 'Mar', income: 1088000, expenses: 792000 },
  { businessId: 'b3', period: 'Apr', income: 1215000, expenses: 903300 },
]

export const CHART_OF_ACCOUNTS: ChartOfAccountEntry[] = [
  {
    id: 'coa-b1-101',
    businessId: 'b1',
    code: '1010',
    name: 'Cash at Bank',
    category: 'Asset',
    balance: 186500,
  },
  {
    id: 'coa-b1-102',
    businessId: 'b1',
    code: '1020',
    name: 'Merchant Wallet Balance',
    category: 'Asset',
    balance: 28420,
  },
  {
    id: 'coa-b1-201',
    businessId: 'b1',
    code: '2010',
    name: 'Supplier Payables',
    category: 'Liability',
    balance: 44750,
  },
  {
    id: 'coa-b1-301',
    businessId: 'b1',
    code: '3010',
    name: 'Owner Equity',
    category: 'Equity',
    balance: 170000,
  },
  {
    id: 'coa-b1-401',
    businessId: 'b1',
    code: '4010',
    name: 'Daily Sales',
    category: 'Revenue',
    balance: 240000,
    profitLossGroup: 'income',
  },
  {
    id: 'coa-b1-402',
    businessId: 'b1',
    code: '4020',
    name: 'Bulk Orders',
    category: 'Revenue',
    balance: 52000,
    profitLossGroup: 'income',
  },
  {
    id: 'coa-b1-403',
    businessId: 'b1',
    code: '4030',
    name: 'Take Outs',
    category: 'Revenue',
    balance: 64000,
    profitLossGroup: 'income',
  },
  {
    id: 'coa-b1-404',
    businessId: 'b1',
    code: '4040',
    name: 'Other Revenue',
    category: 'Revenue',
    balance: 26000,
    profitLossGroup: 'income',
  },
  {
    id: 'coa-b1-501',
    businessId: 'b1',
    code: '5010',
    name: 'Inventory Purchases',
    category: 'Expense',
    balance: 98000,
    profitLossGroup: 'cost_of_goods_sold',
  },
  {
    id: 'coa-b1-502',
    businessId: 'b1',
    code: '5020',
    name: 'Beverage Stock Cost',
    category: 'Expense',
    balance: 82000,
    profitLossGroup: 'cost_of_goods_sold',
  },
  {
    id: 'coa-b1-503',
    businessId: 'b1',
    code: '5030',
    name: 'Packaging Cost',
    category: 'Expense',
    balance: 12000,
    profitLossGroup: 'cost_of_goods_sold',
  },
  {
    id: 'coa-b1-504',
    businessId: 'b1',
    code: '5040',
    name: 'Stock Shrinkage',
    category: 'Expense',
    balance: 22000,
    profitLossGroup: 'cost_of_goods_sold',
  },
  {
    id: 'coa-b1-601',
    businessId: 'b1',
    code: '6010',
    name: 'Utilities Expense',
    category: 'Expense',
    balance: 18500,
    profitLossGroup: 'operating_expense',
  },
  {
    id: 'coa-b1-602',
    businessId: 'b1',
    code: '6020',
    name: 'Shop Rent',
    category: 'Expense',
    balance: 24000,
    profitLossGroup: 'operating_expense',
  },
  {
    id: 'coa-b1-603',
    businessId: 'b1',
    code: '6030',
    name: 'Staff Wages',
    category: 'Expense',
    balance: 21000,
    profitLossGroup: 'operating_expense',
  },
  {
    id: 'coa-b1-604',
    businessId: 'b1',
    code: '6040',
    name: 'Marketing Expense',
    category: 'Expense',
    balance: 10000,
    profitLossGroup: 'operating_expense',
  },
  {
    id: 'coa-b2-101',
    businessId: 'b2',
    code: '1010',
    name: 'Cash at Bank',
    category: 'Asset',
    balance: 264800,
  },
  {
    id: 'coa-b2-102',
    businessId: 'b2',
    code: '1020',
    name: 'Merchant Wallet Balance',
    category: 'Asset',
    balance: 51240,
  },
  {
    id: 'coa-b2-201',
    businessId: 'b2',
    code: '2010',
    name: 'Trade Payables',
    category: 'Liability',
    balance: 68250,
  },
  {
    id: 'coa-b2-301',
    businessId: 'b2',
    code: '3010',
    name: 'Owner Equity',
    category: 'Equity',
    balance: 244000,
  },
  {
    id: 'coa-b2-401',
    businessId: 'b2',
    code: '4010',
    name: 'Daily Dining Sales',
    category: 'Revenue',
    balance: 284000,
    profitLossGroup: 'income',
  },
  {
    id: 'coa-b2-402',
    businessId: 'b2',
    code: '4020',
    name: 'Catering Revenue',
    category: 'Revenue',
    balance: 96000,
    profitLossGroup: 'income',
  },
  {
    id: 'coa-b2-403',
    businessId: 'b2',
    code: '4030',
    name: 'Take Out Revenue',
    category: 'Revenue',
    balance: 118000,
    profitLossGroup: 'income',
  },
  {
    id: 'coa-b2-404',
    businessId: 'b2',
    code: '4040',
    name: 'Other Revenue',
    category: 'Revenue',
    balance: 50000,
    profitLossGroup: 'income',
  },
  {
    id: 'coa-b2-501',
    businessId: 'b2',
    code: '5010',
    name: 'Food Ingredients',
    category: 'Expense',
    balance: 168000,
    profitLossGroup: 'cost_of_goods_sold',
  },
  {
    id: 'coa-b2-502',
    businessId: 'b2',
    code: '5020',
    name: 'Beverage Inventory',
    category: 'Expense',
    balance: 54000,
    profitLossGroup: 'cost_of_goods_sold',
  },
  {
    id: 'coa-b2-503',
    businessId: 'b2',
    code: '5030',
    name: 'Packaging Supplies',
    category: 'Expense',
    balance: 21000,
    profitLossGroup: 'cost_of_goods_sold',
  },
  {
    id: 'coa-b2-504',
    businessId: 'b2',
    code: '5040',
    name: 'Kitchen Production Supplies',
    category: 'Expense',
    balance: 44000,
    profitLossGroup: 'cost_of_goods_sold',
  },
  {
    id: 'coa-b2-601',
    businessId: 'b2',
    code: '6010',
    name: 'Restaurant Rent',
    category: 'Expense',
    balance: 36000,
    profitLossGroup: 'operating_expense',
  },
  {
    id: 'coa-b2-602',
    businessId: 'b2',
    code: '6020',
    name: 'Utilities Expense',
    category: 'Expense',
    balance: 22500,
    profitLossGroup: 'operating_expense',
  },
  {
    id: 'coa-b2-603',
    businessId: 'b2',
    code: '6030',
    name: 'Staff Salaries',
    category: 'Expense',
    balance: 42000,
    profitLossGroup: 'operating_expense',
  },
  {
    id: 'coa-b2-604',
    businessId: 'b2',
    code: '6040',
    name: 'Promotions and Marketing',
    category: 'Expense',
    balance: 12000,
    profitLossGroup: 'operating_expense',
  },
  {
    id: 'coa-b3-101',
    businessId: 'b3',
    code: '1010',
    name: 'Cash at Bank',
    category: 'Asset',
    balance: 824500,
  },
  {
    id: 'coa-b3-102',
    businessId: 'b3',
    code: '1020',
    name: 'Merchant Wallet Balance',
    category: 'Asset',
    balance: 132900,
  },
  {
    id: 'coa-b3-201',
    businessId: 'b3',
    code: '2010',
    name: 'Accrued Liabilities',
    category: 'Liability',
    balance: 211300,
  },
  {
    id: 'coa-b3-301',
    businessId: 'b3',
    code: '3010',
    name: 'Shareholders Equity',
    category: 'Equity',
    balance: 684000,
  },
  {
    id: 'coa-b3-401',
    businessId: 'b3',
    code: '4010',
    name: 'Wholesale Sales',
    category: 'Revenue',
    balance: 760000,
    profitLossGroup: 'income',
  },
  {
    id: 'coa-b3-402',
    businessId: 'b3',
    code: '4020',
    name: 'Distribution Contracts',
    category: 'Revenue',
    balance: 210000,
    profitLossGroup: 'income',
  },
  {
    id: 'coa-b3-403',
    businessId: 'b3',
    code: '4030',
    name: 'Take Out Supply Revenue',
    category: 'Revenue',
    balance: 165000,
    profitLossGroup: 'income',
  },
  {
    id: 'coa-b3-404',
    businessId: 'b3',
    code: '4040',
    name: 'Other Revenue',
    category: 'Revenue',
    balance: 80000,
    profitLossGroup: 'income',
  },
  {
    id: 'coa-b3-501',
    businessId: 'b3',
    code: '5010',
    name: 'Product Procurement',
    category: 'Expense',
    balance: 420000,
    profitLossGroup: 'cost_of_goods_sold',
  },
  {
    id: 'coa-b3-502',
    businessId: 'b3',
    code: '5020',
    name: 'Logistics Fulfilment',
    category: 'Expense',
    balance: 96000,
    profitLossGroup: 'cost_of_goods_sold',
  },
  {
    id: 'coa-b3-503',
    businessId: 'b3',
    code: '5030',
    name: 'Packaging Materials',
    category: 'Expense',
    balance: 48000,
    profitLossGroup: 'cost_of_goods_sold',
  },
  {
    id: 'coa-b3-504',
    businessId: 'b3',
    code: '5040',
    name: 'Warehouse Supplies',
    category: 'Expense',
    balance: 98000,
    profitLossGroup: 'cost_of_goods_sold',
  },
  {
    id: 'coa-b3-601',
    businessId: 'b3',
    code: '6010',
    name: 'Admin Salaries',
    category: 'Expense',
    balance: 92000,
    profitLossGroup: 'operating_expense',
  },
  {
    id: 'coa-b3-602',
    businessId: 'b3',
    code: '6020',
    name: 'Rent and Utilities',
    category: 'Expense',
    balance: 58000,
    profitLossGroup: 'operating_expense',
  },
  {
    id: 'coa-b3-603',
    businessId: 'b3',
    code: '6030',
    name: 'Fleet Expenses',
    category: 'Expense',
    balance: 43200,
    profitLossGroup: 'operating_expense',
  },
  {
    id: 'coa-b3-604',
    businessId: 'b3',
    code: '6040',
    name: 'Marketing and Promotion',
    category: 'Expense',
    balance: 48100,
    profitLossGroup: 'operating_expense',
  },
]

export const ACCOUNTING_TRANSACTIONS: AccountingTransaction[] = [
  {
    id: 'txn-b1-1',
    businessId: 'b1',
    type: 'receive',
    amount: 8200,
    toAccountId: 'cash-b1-merchant',
    reference: 'RCV-1001',
    description: 'DirectPay wallet collections',
    createdAt: '2026-03-18T09:30:00.000Z',
  },
  {
    id: 'txn-b1-2',
    businessId: 'b1',
    type: 'transfer',
    amount: 15000,
    fromAccountId: 'cash-b1-merchant',
    toAccountId: 'cash-b1-bank',
    reference: 'TRF-1002',
    description: 'Wallet settlement to bank',
    createdAt: '2026-03-18T13:10:00.000Z',
  },
  {
    id: 'txn-b1-3',
    businessId: 'b1',
    type: 'send',
    amount: 4200,
    fromAccountId: 'cash-b1-bank',
    toAccountId: 'coa-b1-501',
    reference: 'SND-1003',
    description: 'Utility expense payment',
    createdAt: '2026-03-19T08:15:00.000Z',
  },
  {
    id: 'txn-b2-1',
    businessId: 'b2',
    type: 'receive',
    amount: 12400,
    toAccountId: 'cash-b2-merchant',
    reference: 'RCV-2001',
    description: 'Merchant payout receipts',
    createdAt: '2026-03-18T10:20:00.000Z',
  },
  {
    id: 'txn-b2-2',
    businessId: 'b2',
    type: 'transfer',
    amount: 22000,
    fromAccountId: 'cash-b2-merchant',
    toAccountId: 'cash-b2-bank',
    reference: 'TRF-2002',
    description: 'Settlement to operations bank',
    createdAt: '2026-03-19T11:45:00.000Z',
  },
  {
    id: 'txn-b2-3',
    businessId: 'b2',
    type: 'send',
    amount: 5600,
    fromAccountId: 'cash-b2-bank',
    toAccountId: 'coa-b2-501',
    reference: 'SND-2003',
    description: 'Facility and utility costs',
    createdAt: '2026-03-20T07:40:00.000Z',
  },
  {
    id: 'txn-b3-1',
    businessId: 'b3',
    type: 'receive',
    amount: 28400,
    toAccountId: 'cash-b3-bank',
    reference: 'RCV-3001',
    description: 'Wholesale bank deposits',
    createdAt: '2026-03-18T14:10:00.000Z',
  },
  {
    id: 'txn-b3-2',
    businessId: 'b3',
    type: 'transfer',
    amount: 50000,
    fromAccountId: 'cash-b3-bank',
    toAccountId: 'cash-b3-merchant',
    reference: 'TRF-3002',
    description: 'Working capital moved to merchant wallet',
    createdAt: '2026-03-19T12:00:00.000Z',
  },
  {
    id: 'txn-b3-3',
    businessId: 'b3',
    type: 'send',
    amount: 18250,
    fromAccountId: 'cash-b3-bank',
    toAccountId: 'coa-b3-501',
    reference: 'SND-3003',
    description: 'Operating expense settlement',
    createdAt: '2026-03-20T09:05:00.000Z',
  },
]
