...
export function SalesQuotationDetailPage() {
  ...
  const [row, setRow] = useState<SalesQuotationRow | null>(null)
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  const [pdfBusy, setPdfBusy] = useState(false)

  useEffect(() => {
    if (!businessId || !quotationId) return
    setLoading(true)
    setError(null)
    void fetchSalesQuotation(businessId, quotationId)
      .then(setRow)
      .catch((e) =>
  ...
  const handlePdf = useCallback(async () => {
    ...
    setPdfBusy(true)
    try {
      await downloadSalesQuotationPdf(businessId, quotationId)
    } finally {
      setPdfBusy(false)
    }
  }, [businessId, quotationId])
