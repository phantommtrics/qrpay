...
import { APP_PATHS, platformMerchantJournalDetailPath } from '../../config/navigation'
import { useAuth } from '../../features/auth/AuthContext'
import {
  ApiError,
  fetchMerchantJournalEntries,
  type MerchantJournalListRow,
...
export function PlatformMerchantJournalEntriesPage() {
  const { canAccess } = useAuth()
  const allowed = canAccess('platform.accounting.transaction_journal')

  const [from, setFrom] = useState('')
  const [to, setTo] = useState('')
  const [businessId, setBusinessId] = useState('')
  const [page, setPage] = useState(1)
  const [rows, setRows] = useState<MerchantJournalListRow[]>([])
  ...
  const load = useCallback(() => {
    ...
    setLoading(true)
    setError(null)
    void fetchMerchantJournalEntries({
      page,
      pageSize: PAGE_SIZE,
  ...
  }, [allowed, page, businessId, from, to])
  ...
  return (
    <PageTransition>
      <FinanceReportChrome
        ...
        description="All merchant general-ledger postings. Customer QR / POS sale and wallet-fee journals are exempt from approval and always flow to statements. Other postings require approval before they appear on GL, P&amp;L, and account statements."
        toolbar={null}
      >
        <PageCard
          variant="default"
                    ...
                    {rows.length === 0 ? (
                      ...
                      rows.map((r) => (
                        <tr key={r.id} className="align-top hover:bg-qb-surface/40">
                          ...
                          <td className="max-w-[200px] px-3 py-2 text-qb-heading">
                            <span className="line-clamp-2">{r.businessName}</span>
                            <span className="mt-0.5 block font-mono text-[10px] text-qb-muted">
                              {r.businessId.slice(0, 10)}…
                            </span>
                          </td>
                          ...
                          <td className="max-w-[280px] px-3 py-2 text-xs text-qb-muted">
                            <span className="line-clamp-2">{r.memo ?? '—'}</span>
                          </td>
                          <td className="px-3 py-2 text-right">
                            <Link
                              to={platformMerchantJournalDetailPath(r.id)}
