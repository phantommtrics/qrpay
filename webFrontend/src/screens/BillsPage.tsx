...
import { APP_PATHS, salesBillDetailPath } from '../config/navigation'
...
function linesTotalApi(lines: BillRow['lines']): number {
...
export function BillsPage() {
  ...
  const hydrateFromBill = useCallback((bill: BillRow) => {
    setIssueDate(bill.issueDate.slice(0, 10))
    setDueDate(bill.dueDate ? bill.dueDate.slice(0, 10) : '')
    setReference(bill.reference ?? '')
    setContactId(bill.contactId)
    setContactInput(bill.contact.name)
    const ordered = [...bill.lines].sort((a, b) => a.sortOrder - b.sortOrder)
    setLines(
      ordered.length
        ? ordered.map((l) => ({
            id: newLineId(),
  ...
  const startEditBill = (inv: BillRow) => {
    setEditingBillId(inv.id)
    hydrateFromBill(inv)
    setTab('new')
    setError(null)
    setToast(null)
  ...
  const openMarkPaid = (inv: BillRow) => {
    setPayTarget(inv)
    setPaySettlementId('')
  ...
  const submitMarkPaid = async (e: FormEvent) => {
    ...
    setToast(null)
    try {
      await markBillPaid(businessId, payTarget.id, {
        settlementChartAccountId: paySettlementId,
        postedAt: dateInputToIso(payPostedAt),
      })
      setPayTarget(null)
      setToast({ message: 'Payment recorded; journal posted (money out).', variant: 'success' })
      loadList()
    } catch (err) {
      const msg =
  ...
  const submitNew = async (e: FormEvent) => {
    ...
    try {
      if (editingBillId) {
        await patchBill(businessId, editingBillId, {
          contactId,
        ...
        setToast({ message: 'Bill draft updated.', variant: 'success' })
      } else {
        await createBill(businessId, {
          contactId,
          issueDate: dateInputToIso(issueDate),
          dueDate: dueDate.trim() ? dateInputToIso(dueDate.trim()) : null,
          reference: reference.trim() || null,
        ...
        setToast({ message: 'Bill saved as draft.', variant: 'success' })
      }
      resetForm()
      loadList()
    } catch (err) {
    ...
    <PageTransition>
      ...
      <div className="space-y-5 py-2 lg:space-y-6">
        ...
        >
          ...
          <div>
            <h1 className="text-2xl font-semibold tracking-tight text-qb-heading">Bills</h1>
            <p className="mt-2 max-w-2xl text-sm leading-relaxed text-qb-muted">
              Purchase bills record what you owe suppliers. Drafts can be edited; approving emails the
              supplier contact and requires their email. The ledger posts when you mark a bill paid
              (cash-basis, money out) — choose the bank or cash account and payment date.
            </p>
          </div>
        ...
        {tab === 'list' ? (
          ...
          >
            ...
            {loadingList ? (
              ...
              <p className="text-sm text-qb-muted">No bills yet.</p>
            ) : (
                            ...
                            <div className="flex flex-wrap gap-2">
                              ...
                              {canApprove ? (
                                <button
                                  ...
                                  title={
                                    !hasEmail
                                      ? 'Add an email to this supplier contact before approving (notification is sent by email).'
                                      : undefined
                                  }
                                  onClick={() =>
                                    void runRowAction(
                                      inv.id,
                                      () => approveBill(businessId, inv.id),
                                      'Bill approved and supplier notified.',
                                    )
                                  }
                              ...
                              {canVoid ? (
                                <button
                                  ...
                                  disabled={busyRow}
                                  onClick={() =>
                                    void runRowAction(inv.id, () => voidBill(businessId, inv.id), 'Bill voided.')
                                  }
          ...
          >
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 rounded-md border border-amber-200 bg-amber-50/90 px-4 py-3 text-sm text-amber-950">
                <p>
                  ...
                  <span className="font-mono font-semibold">
                    {rows.find((r) => r.id === editingBillId)?.publicCode ?? 'bill'}
                  </span>
            ...
            <form noValidate onSubmit={(e) => void submitNew(e)} className="space-y-6">
              ...
              <div className="flex flex-wrap items-end justify-between gap-4 border-t border-qb-border pt-5">
                ...
                >
                  {busy ? 'Saving…' : editingBillId ? 'Save changes' : 'Save draft'}
                </button>
              </div>
            </form>
          </PageCard>
