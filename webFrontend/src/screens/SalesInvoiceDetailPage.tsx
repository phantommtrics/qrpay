...
export function SalesInvoiceDetailPage() {
  ...
  const [row, setRow] = useState<SalesInvoiceRow | null>(null)
  ...
  useEffect(() => {
    ...
    setLoading(true)
    setError(null)
    void fetchSalesInvoice(businessId, invoiceId)
      .then(setRow)
      .catch((e) =>
  ...
  const handlePdf = useCallback(async () => {
    ...
    setPdfBusy(true)
    try {
      await downloadSalesInvoicePdf(businessId, invoiceId)
    } finally {
      setPdfBusy(false)
    }
  }, [businessId, invoiceId])
