...
export function GlBalanceReportPage() {
  ...
  const [data, setData] = useState<GlBalanceReportData | null>(null)
  ...
  const load = useCallback(() => {
    ...
    setLoading(true)
    setError(null)
    void fetchGlBalanceReport(businessId, asOf)
      .then(setData)
  ...
  const exportCsv = () => {
    ...
    downloadCsv(`gl-balance-${asOf}.csv`, headers, rows)
  }

  const exportPdf = async () => {
    if (!data || !currentOrganization?.name) return
    await downloadFinancePdf({
      title: 'GL balance (trial balance)',
  ...
  return (
    <PageTransition>
      <FinanceReportChrome
        ...
        description="Xero-style trial balance: every general ledger account with cumulative debits, credits, and balance as at the date you choose. Total debits should equal total credits when books are balanced."
        toolbar={
          <ReportExportToolbar
            canExport={canExport}
          ...
          {data ? (
            <>
              ...
              <p className="text-xs text-qb-muted">
                Tip: a non-zero difference usually means an unbalanced journal — review recent entries.
              </p>
            </>
          ) : null}
        </PageCard>
